...
const DataGrid = () => {
    ...
    const getData = async () => {
        ...
        try {
            ...
            const responseposts = await fetch(`${apiServices.comments}`);
            const responseDatapost = await responseposts.json();
            setSectableData(responseDatapost)
            if (responseData.length > 0) {
                const firstItem = responseData[0];
                const allowedKeys = ['id', 'body'];
